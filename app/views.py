...
def train_model_polynomial():
    ...
    joblib.dump((model, scaler), 'ml_polynomial_regression_model.pkl')
...
def predict_diabetes_polynomial():
    model, scaler = joblib.load('ml_polynomial_regression_model.pkl')
...
def train_model_gradient():
    ...
    # Сохранение обученной модели для последующего использования
    joblib.dump((model, scaler), 'ml_gradient_boosting_model.pkl')
...
def predict_diabetes_gradient():
    ...
    model, scaler = joblib.load('ml_gradient_boosting_model.pkl')
...
def train_model_recurrent():
    ...
    model.save_weights('ml_rnn_model.weights.h5')
...
def predict_diabetes_recurrent():
    ...
    if request.method == 'POST':
        ...
        if model is None:
            ...
            model.load_weights('ml_rnn_model.weights.h5')
